...
import { Movie, TVShow, MediaItem, CastMember, Video, MovieResponse, TVResponse } from '../../core/models';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, CastListComponent, VideoPlayerComponent, ContentSliderComponent],
  template: `
...
export class DetailsComponent implements OnInit {
  ...
  private loadMedia(type: 'movie' | 'tv', id: number) {
    ...
    // Load media details
    const details$: Observable<Movie | TVShow> = type === 'movie' 
      ? this.tmdbApi.getMovieDetails(id)
      : this.tmdbApi.getTVDetails(id);
    ...
    details$.subscribe({
      next: (media: Movie | TVShow) => {
        ...
        const cast$ = type === 'movie'
          ? this.tmdbApi.getMovieCast(id)
          : this.tmdbApi.getTVCast(id);
        ...
        const videos$ = type === 'movie'
          ? this.tmdbApi.getMovieVideos(id)
          : this.tmdbApi.getTVVideos(id);
        ...
        // Load similar
        const similar$: Observable<MovieResponse | TVResponse> = type === 'movie'
          ? this.tmdbApi.getSimilarMovies(id)
          : this.tmdbApi.getSimilarTVShows(id);
